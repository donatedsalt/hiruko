...
- security(headers): add `Content-Security-Policy-Report-Only` to `next.config.ts` alongside the existing baseline headers. Directives kept reasonably strict (`default-src 'self'`, `frame-ancestors 'none'`, `object-src 'none'`, `base-uri 'self'`, `form-action 'self'`) with explicit allowlists for Clerk (`*.clerk.com`, `*.clerk.accounts.dev`, `clerk-telemetry.com`, `*.clerk-telemetry.com`, image origins `img.clerk.com` / `images.clerk.dev`), Convex (`https://*.convex.cloud` + `wss://*.convex.cloud` for the realtime client and `https://*.convex.site` for HTTP routes like `/clerk-webhook`), and Vercel Speed Insights (`va.vercel-scripts.com`). `script-src` keeps `'unsafe-inline'` + `'unsafe-eval'` for Next's inline runtime; `style-src` keeps `'unsafe-inline'` for Tailwind v4 primitives. Report-only on purpose — browser logs violations to console with no `report-uri`/`report-to` collector wired up; promote to enforcing `Content-Security-Policy` once the report stream is clean
- fix(transactions/new): decouple the type toggle from the category select so neither silently overwrites the other. Both `onValueChange` handlers now set only their own state, and the derived `txnType` default reads the _currently selected_ category's type (not `categories[0]`) so the pre-touch default still matches whatever category is shown. Manually toggling Income after picking an expense category is preserved across further category changes; user sees the existing "Category type and transaction type must match" mutation error if they submit a mismatched combo. Full decoupling of the Convex-level `category.type === txn.type` constraint is a larger TODO
- fix(dates): root-cause the "invalid date error" along the full create→persist→display path. `DatePicker` now always emits its hidden input when `name` is set (even with an empty value when no date is picked), so cleared selections aren't silently dropped from the form payload. `TransactionSchema.transactionTime` adds `.refine(Number.isFinite, ...)` — Zod's plain `z.number()` accepts `NaN` (which is what `new Date("nullT12:34").getTime()` produces when the date input is empty), so the schema was the actual choke point that let bad timestamps reach the DB. Defensive guards on the read path: `transactions/[id]/page.tsx` checks `Number.isFinite(txnDate.getTime())` before `.toISOString()` (the prior throw was the visible "invalid date" RangeError when opening an already-bad row), and `transaction-list.tsx` `groupByDay` / `formatDisplayDate` / the date sort all fall back to an `"Unknown date"` bucket pinned to the bottom instead of crashing the list
- refactor(layout): single `src/app/layout.tsx` owns the only `<html>`/`<body>`, the Geist + Geist_Mono fonts, `globals.css`, the provider stack (`ClerkProvider` → `ConvexClientProvider` → `ThemeProvider`), and the shared `metadata`. `(auth)/layout.tsx` slims to `{children}` + the fixed `<ThemeChangeButton>`. `(dashboard)/layout.tsx` slims to its group chrome only (`CommandBarProvider` → `SidebarProvider` + `AppSidebar` + `SidebarInset`, plus `CommandBar`/`Toaster`/`FloatingButtons`/`HistoryTracker`/`SpeedInsights`). Eliminates the duplicated provider stack between the two route groups so future provider additions land in one place. AGENTS.md gotcha updated to "Single root layout"
- security(api/chat): per-IP rate-limit fallback on top of the existing per-user window. Pulls the client IP from `x-forwarded-for` (then `x-real-ip`), hashes it with a per-process salt via `crypto.subtle.digest("SHA-256", ...)` so the in-memory bucket map isn't reversible from a memory dump, and caps at 60/min/IP — 3× the per-user 20/min, accommodating shared NATs while still throttling the multi-account workaround. Both checks run; either firing returns 429 with `Retry-After: 60`. Caveat in source: in-memory + per-process means it doesn't synchronize across serverless instances; treat as best-effort defence in depth
- chore(metadata): per-route `<title>` for every dashboard page. Eight tiny segment layouts (`transactions/`, `transactions/new/`, `transactions/[id]/`, `categories/`, `budgets/`, `goals/`, `statistics/`, `consult/`) each export `{ title: "<Page>" }` and pass `children` through. The `(dashboard)` root layout's existing `title.template = "%s | Hiruko"` joins them, e.g. `/categories` → "Categories | Hiruko". Home (`/`) keeps the bare "Hiruko" default. Page components stay `"use client"` (the metadata lives in the parallel server-only layout file)
- perf(convex): cap unbounded `.collect()` calls per the Convex guideline. `transactions.queries.statsByDay` reads at most 5,000 txns from the chart window. `users.mutations.initializeUser` and `categories.mutations.createDefaultCategories` only need to know if 0/1/2/3+ categories exist, so both `.take(3)` and gate the "balance correction only" branch on `length <= 2`. Cascade deletes (`accounts.remove`, `categories.remove`, `budgets.remove`, `goals.remove`) `.take(MAX_CASCADE_TRANSACTIONS = 5000)` and throw a clear `cascadeCapError(entity)` if at the cap, surfaced as a toast — keeps writes within Convex's per-mutation transaction limit instead of silently orphaning rows. Shared cap + helper live in new `convex/utils/limits.ts`. Future scale: switch to batch + `ctx.scheduler.runAfter` (TODO retained)
